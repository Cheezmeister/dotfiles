$ ->
  ...
  $('#convertbutton').click ->
    ...
    # This one wasn't.
      replace(/^> (.*)/gmi                                 , "[quote]$1[/quote]")
    $textbox.val(text)

  # Compact topic listings
  $('td.windowbg2 span.smalltext br').remove()
  $('td.windowbg2 span.smalltext').map ->
    lines = @innerHTML.split "\n"
    @innerHTML = "#{new Date(lines[1]).getFullYear() || 'Today'} #{lines[2]}"
